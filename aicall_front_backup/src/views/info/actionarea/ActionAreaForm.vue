                    ...
                    <div class="row">
                        <div class="col-12">
                            <b-form-input class="short"
                                          type="text"
                                          required
                                          v-model="actionarea.item.name_ru"
                            />
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <b-form-input class="short"
                                          type="text"
                                          required
                                          v-model="actionarea.item.name_eng"
                            />
                        </div>
                    </div>
                </div>
...
export default {
    ...
    created() {
        ...
        if (this.$route.params.id) {
            this.$store.dispatch('actionarea/getItem', this.$route.params.id)
                .then(item => {
                    console.log(item)
                })
                .catch(error => {
    ...
    methods: {
        goSave(e){
            e.preventDefault();
            let data = Object.assign({}, this.actionarea.item);
            data.id = this.id
            this.$store.dispatch('actionarea/saveItem',data)
                .then(item => {
                    console.log(item)
                    Vue.templateShowSuccess();
                    if(!data.id) Vue.goBack();
                    else this.$store.dispatch('actionarea/getItem', this.$route.params.id)
                            .then(item => {
                                console.log(item)
                            })
                            .catch(error => {
